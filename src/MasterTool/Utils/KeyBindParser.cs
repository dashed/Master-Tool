...
namespace MasterTool.Utils
{
    ...
    public static class KeyBindParser
    {
        ...
        /// Checks aliases first, then tries KeyCode enum parse, then single letter.
        /// </summary>
        internal static bool TryParseKeyName(string name, out KeyCode keyCode)
        {
            ...
            if (Enum.TryParse(name, true, out keyCode) && Enum.IsDefined(typeof(KeyCode), keyCode))
                return true;

            // Single letter → KeyCode.A through KeyCode.Z (KeyCode uses lowercase ASCII 97-122)
